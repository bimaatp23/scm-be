...
inventoryRouter.post('/create', multer().none(), authenticateJwt(['admin'], ['item_name', 'description', 'unit', 'price']), async (req, res) => {
    inventoryModel.create(req, (err, resp) => {
        return res.status(resp.error_schema.error_code).json(resp)
    })
})

inventoryRouter.post('/update', multer().none(), authenticateJwt(['admin'], ['id', 'item_name', 'description', 'unit', 'price']), async (req, res) => {

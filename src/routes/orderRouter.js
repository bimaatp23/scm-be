...
orderRouter.get("/list", authenticateJwt([BasicConstant.ROLE_ADMIN, BasicConstant.ROLE_PENGADAAN, BasicConstant.ROLE_GUDANG, BasicConstant.ROLE_PRODUKSI, BasicConstant.ROLE_DISTRIBUSI, BasicConstant.ROLE_RETAIL]), async (req, res) => {
...
orderRouter.post("/done", multer().none(), authenticateJwt([BasicConstant.ROLE_DISTRIBUSI]), checkRequest(["order_id", "done_date"]), async (req, res) => {
    new orderModel().done(req, (err, resp) => {
        return res.status(resp.error_schema.error_code).json(resp)
    })
})